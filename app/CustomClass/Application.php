...
class Application
{
    ...
    public function showApplication($id){
        ...
        $student = StudentList::get_student_id_from_application_id($this->application_id);

        $data = ApplicationModel::where('applications.student_id',$student->id)
                                    ->where('applications.id', $this->application_id)
                                    ->join('course_applieds', 'course_applieds.id', '=', 'applications.course_applied_id')
                                    ->select('course_applieds.course_id','course_applieds.university_id','course_applieds.campus')
                                    ->first();
        return $data;
    }
    ...
    public function updateApplicationStatusOfStudent($data){
        ...
        $this->sendEmail($data);
        $resp =  ApplicationStatus::create($this->applicationStatus);
        if($resp->id){
            ...
            \App\Application::UpdateStatus($data['application_id'],$data['category']);
        }else
...
//          Mail::to($studentDetails->email)->send(new \App\Mail\application\ReadytoFileVisa($studentDetails,$course_details,$university_details));
    ...
    private function checkStatusUpdate()
    {
        ...
        {
            $this->currentStage  = StudentList::current_stage('application_id',$this->applicationStatus['application_id']);
            ...
            if($this->currentStage < $current_id){
                // update status
                 $student = StudentList::get_student_id_from_application_id( $this->applicationStatus['application_id']);
                 $stage      = Stage::where('student_id', $student->id)->update(['stage' => $current_id]);
                // stage update
                if($stage)
                    return true;
                else
    ...
    private function AssignDocumentsToStatus($application_status_id,$application_id,$documents){
        foreach($documents as $key => $value){
            ...
            ApplicationStatusDocument::create($data);
        }
    }
    // application id //
