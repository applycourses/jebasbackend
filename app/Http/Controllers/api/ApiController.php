...
class ApiController extends Controller
{
     ...
     public function agent_application_document_by_application_id(Request $request){
         if($request->ajax()){
             $id                  = $request->id;
             $application         = AgentApplication::select('id','course_applied_id','student_id')->find($id);
             $course_details      = AgentCourseApplied::find($application->course_applied_id);

             $data                = AgentDocuments::where('applied_course_id',$course_details->course_id)
                                         ->where('category_id','12')
                                         ->where('student_id',$application->student_id)
                                         ->get();
             foreach($data as $key => $value ){
                 $data[$key]['document'] = AgentDocuments::DocumentName($value->document_id);
                 $data[$key]['download'] = Storage::disk('s3')->url($value->path);
             }
             return response()->json($data);
         }else{
     ...
     public function agentpaymentStatusOfStudent(Request $request){
             $id                  = $request->id;
             $application         = AgentApplication::select('tution_fee')->find($id);
             return response()->json($application);
     }
      public function agentupdatePaymentStatusOfStudent(Request $request){
         $id            = $request->id;
         $tution_fee    = $request->tution_fee;
    ...
    public function agentviewAcceptanceLetterOfStudent(Request $request){
         $id                  = $request->id;
         $application         = AgentApplication::select('acceptance')->find($id);
         return response()->json($application);
     }
     public function agentupdateAcceptanceStatusOfStudent(Request $request){
        $id            = $request->id;
        $acceptance    = $request->acceptance;
        $application   = AgentApplication::find($id);
        $response      = $application->update(['acceptance' => $acceptance]);
